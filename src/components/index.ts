...
export function initHomePage():void {
	...
	const handlePayment = (name:string,price:string | number) => {
		...
		const paymentData:IamportPaymentOptions = {
			pg:'html5_inicis',pay_method:'card',merchant_uid:orderId,name,amount:parseInt(String(price),10) * 1000,buyer_email:'honggildong@example.com',buyer_name:'홍길동',buyer_tel:'01012345678',buyer_addr:'서울특별시 강남구 테헤란로 123',buyer_postcode:'06130',m_redirect_url:'https://gurumauto.cafe24.com/',
		}
		;
		IMP.request_pay(paymentData,function (rsp:any) {
			...
			if (rsp.success) {
				...
				fetch('/', {
					...
					if (data.success) {
						resultDiv.innerHTML = <h2 class="success">✅ 결제가 정상적으로 완료되었습니다.</h2><p>주문번호:$ {
							rsp.merchant_uid
						}
						</p><p>결제 금액:$ {
							rsp.paid_amount
						...
						원</p>;
						// ✅ 자동 POST 전송 및 페이지 이동 const form = document.createElement('form');
						form.method = 'POST';
						...
						resultDiv.innerHTML = <h2 class="error">❌ 결제 검증 실패</h2><p>메시지:$ {
							data.message
						}
						</p>;
					}
					...
					resultDiv.innerHTML = <h2 class="error">❌ 서버 오류로 결제 검증 실패</h2><p>$ {
						error
					}
					</p>;
				}
				...
				resultDiv.innerHTML = <h2 class="error">❌ 결제에 실패했습니다</h2><p>실패 사유:$ {
					rsp.error_msg
				}
				</p>;
			}
	...
	;
	// ✅ 1. URL 파라미터 방식 처리 if (productName && totalPrice && !isNaN(parseInt(totalPrice,10))) {
		handlePayment(productName,totalPrice);
		return;
		// 리스너 생략
	}
	// ✅ 2. 메시지(postMessage) 방식도 여전히 지원 window.addEventListener('message',(event) => {
